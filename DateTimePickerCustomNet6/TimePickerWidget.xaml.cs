...
{    /// <summary>
    ...
    public class TimeOnlyConverter : IValueConverter
    {
        ...
        {
            if (TimeOnly.TryParse(value.ToString(), out var time))
                return time;
            return value;
        }
    }
    ...
    {
//===================   Properties  ===============================================================================================
        private TimeOnly _activeTime = TimeOnly.FromDateTime(DateTime.Now);  // Property to hold the Active Time (first row)
        public TimeOnly ActiveTime
        {
        ...
        private TimeOnly _selectedTime = TimeOnly.MinValue; // Property to hold the selected Time (second row)
...
//======================    EVENT HANDLERS    ===============================================
        ...
        private void NumberInputTextbox(object sender, TextCompositionEventArgs e) 
        {
            ...
            if (sender is TextBox textBox) //Access the textBox
            {
                ...
                {
                    e.Handled = true;
                    textBox.Text = string.Concat(textBox.Text.Substring(0, selectionStart), "0", e.Text, textBox.Text.AsSpan(selectionStart + selectionLength));
                ...
                else if (isSecondCall)// If the input is a two-digit (meaning two events triggred closely)
                {
                    ...
                    {// write down the input of the keyboard to the textbox text that was selected, leaving the rest text in place
                        textBox.Text = string.Concat(textBox.Text.Substring(0, selectionStart), timeInt.ToString(), textBox.Text.AsSpan(selectionStart + selectionLength));
                    }
                    else
                    {// If the input was out of bounds, the input becomes the max value that is appropriate
                        textBox.Text = string.Concat(textBox.Text.Substring(0, selectionStart), MaxTimeVals[selectionStart / 3].ToString(), textBox.Text.AsSpan(selectionStart + selectionLength));
                    }
                    textBox.SelectionStart = selectionStart;
                    textBox.SelectionLength = selectionLength;
                }
        ...
        {                                                          //When clicked, the textBox should not lose focus an d keep the selected text part
            ...
            if (selectionStart < 8) //Only for Hrs, Mins, Seconds
            {
                ...
                {
                    TimeSelector.Text = string.Concat(TimeSelector.Text.Substring(0, TimeSelector.SelectionStart), "0", timeInt.ToString(), TimeSelector.Text.AsSpan(TimeSelector.SelectionStart + TimeSelector.SelectionLength));
                }
                else if (timeInt >= 10)
                {
                    TimeSelector.Text = string.Concat(TimeSelector.Text.Substring(0, TimeSelector.SelectionStart), timeInt.ToString(), TimeSelector.Text.AsSpan(TimeSelector.SelectionStart + TimeSelector.SelectionLength));
                }
                else
                {
                    TimeSelector.Text = string.Concat(TimeSelector.Text.Substring(0, TimeSelector.SelectionStart), MaxTimeVals[selectionStart / 3].ToString(), TimeSelector.Text.AsSpan(TimeSelector.SelectionStart + TimeSelector.SelectionLength));
                }
            }
            else //For AM/PM
            {
        ...
        private void increaseBtn(object sender, RoutedEventArgs e) //Same logic for increase button
        {
            ...
            if (selectionStart < 8)
            {
                ...
                {   
                    if (selectionStart == 0)
                    {
                        TimeSelector.Text = string.Concat(TimeSelector.Text.Substring(0, TimeSelector.SelectionStart), "01", TimeSelector.Text.AsSpan(TimeSelector.SelectionStart + TimeSelector.SelectionLength));
                    }
                    else
                    {
                        TimeSelector.Text = string.Concat(TimeSelector.Text.Substring(0, TimeSelector.SelectionStart), "00", TimeSelector.Text.AsSpan(TimeSelector.SelectionStart + TimeSelector.SelectionLength));
                    }
